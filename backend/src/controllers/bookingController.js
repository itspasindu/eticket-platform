  ...
  try {
    ...
    if (seats.length !== seatIds.length) {
      return res.status(400).json({
        message:
          "Seats are not locked by you or lock has expired. Please select seats again.",
      });
    }
...
const confirmBooking = async (req, res) => {
  ...
  try {
    // We require here to avoid circular dependency
    const { sendTicketEmail } = require("./ticketController");
    // Trigger email in background — don't await (don't make user wait)
    sendTicketEmailHelper(booking._id, booking, seats);
  } catch (emailError) {
