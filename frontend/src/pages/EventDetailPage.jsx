...
const EventDetailPage = () => {
  ...
  // Fetch seat map
  const { data: seatData, isLoading: seatLoading, refetch: refetchSeats } = useQuery({
    queryKey: ['seats', id],
  ...
  const selectedSeatDetails = allSeats.filter((s) => selectedSeats.includes(s._id));
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      ...
      const { data } = await createBookingAPI({ eventId: id, seatIds: selectedSeats });

      // Step 3: Simulate payment (we'll add Stripe later)
      await confirmBookingAPI(data.booking.id, { paymentId: 'simulated_payment' });
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        ...
        <div className="lg:col-span-2">
          ...
          <h1 className="text-4xl font-extrabold text-white mt-3 mb-4">{event?.title}</h1>
          ...
          <div className="card sticky top-24">
            ...
            <div className="border-t border-gray-700 pt-4 mb-6">
              <div className="flex justify-between font-bold text-lg">
                ...
                <span className="text-primary">LKR {totalAmount.toLocaleString()}</span>
              </div>
            </div>
